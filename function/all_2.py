...
# ฟังก์ชันสำหรับแปลงไฟล์ JSON เป็น XLSX และบันทึกไว้ในโฟลเดอร์ 'exel'
def convert_json_to_xlsx():
    try:
        # เลือกไฟล์ .json โดยใช้ฟังก์ชันที่มีอยู่แล้ว
        list_files_in_folder()  # เรียกฟังก์ชันเพื่อแสดงไฟล์ในโฟลเดอร์
        file_choice = colored_input2("📂 กรุณาเลือกหมายเลขไฟล์ที่ต้องการแปลง: ").strip()

        # ตรวจสอบว่าผู้ใช้เลือกหมายเลขไฟล์
        ...
        # ตรวจสอบว่าไฟล์ที่เลือกเป็นไฟล์ .json
        if not filename.endswith('.json'):
            print_error("❌ กรุณาเลือกไฟล์ .json ❌")
            print_line()
            return
        ...
        # ตรวจสอบว่าโฟลเดอร์ 'exel' มีอยู่หรือไม่
        if not os.path.exists(EXCEL_FOLDER):
            os.makedirs(EXCEL_FOLDER)
            print_header(f"\n🎉 สร้างโฟลเดอร์ {EXCEL_FOLDER} เรียบร้อยแล้ว 🎉")
            print_line()
        ...
        print_error("❌ ไม่สามารถอ่านข้อมูลจากไฟล์ .json ได้ กรุณาตรวจสอบเนื้อหาไฟล์ ❌")
        print_line()
    except PermissionError:
        print_error("❌ ไม่สามารถบันทึกไฟล์ในโฟลเดอร์ 'exel' ได้ กรุณาตรวจสอบสิทธิ์การเข้าถึง ❌")
        print_line()
...
# ฟังก์ชันลบไฟล์จากโฟลเดอร์
def delete_file_from_folder():
    # แสดงรายชื่อไฟล์ในโฟลเดอร์ 'check'
    print_header("📂 --- ไฟล์ในโฟลเดอร์ 'check' --- 📂\n")
    files = os.listdir(FOLDER_NAME)
    if files:
        for idx, file in enumerate(files, 1):
            print_number(f"{idx}. {file}")
        while True:
            try:
                file_index = int(colored_input("\nกรุณาเลือกหมายเลขไฟล์ที่ต้องการลบ: "))
                # ตรวจสอบว่าไฟล์ที่เลือกมีอยู่
                selected_file = files[file_index - 1]
                file_path = os.path.join(FOLDER_NAME, selected_file)
                break
            except (ValueError, IndexError):
                print_alarm("⚠️ กรุณากรอกหมายเลขที่ถูกต้อง! ⚠️")

        # ยืนยันการลบไฟล์
        confirmation = colored_input2(f"คุณต้องการลบไฟล์ {selected_file} หรือไม่? (y/n): ").lower()
        if confirmation == 'y':
            os.remove(file_path)
            print_complete(f"✅ ไฟล์ {selected_file} ถูกลบเรียบร้อยแล้ว! ✅")
        else:
